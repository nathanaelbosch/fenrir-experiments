...
boxplot!(ax, xs, prmses; dodge=dodge, color=[COLORS[d+1] for d in dodge], BOXPLOT_KWARGS...)

labels = ["RK", "FENRIR"]
elements = [PolyElement(polycolor=COLORS[i+1]) for i in 1:length(labels)]
title = "Method"
axislegend(ax, elements, labels, "", position=:lt, margin=(5, 0, 0, 0))

save(joinpath(DIR, "figure5.pdf"), fig, pt_per_unit=1)
