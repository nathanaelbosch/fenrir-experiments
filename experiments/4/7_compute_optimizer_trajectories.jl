...
for p0 in (0.5, 2.0, 5.0, 9.0)
    ...
    plot_callback = function ()
        ...
        function (x, l, times, states)
            ...
            plt = scatter(tsteps, ProbNumDiffEq.stack(noisy_ode_data), label="",
            ...
            plot!(times, smoothed_means, ribbon=2smoothed_stds,
                  fillalpha=0.1,
                  color=[2 1],
                  label="",
    ...
    optprob = OptimizationProblem(f, [log(κ²0)], opt_params; lb=[log(1e-20)], ub=[log(1e50)])
    _cb = (x, l, args...) -> begin
...
using JLD
save("experiments/4/trajectories.jld", "trajectories", trajectories)
