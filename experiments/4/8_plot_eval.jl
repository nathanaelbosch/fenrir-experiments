...
df_rk = CSV.read("experiments/4/rk_df.csv", DataFrame)
# df_rk = df_rk[.!isnan.(df_rk.popt), :]
df_fenrir = CSV.read("experiments/4/fenrir_df.csv", DataFrame)
...
sc1 = scatter!(ax, df_fenrir.p0, df_fenrir.popt,
               markersize=8,
               color=(COLORS[3], 0.8),
               marker=:circle,
               )
sc2 = scatter!(ax, df_rk.p0, df_rk.popt,
               markersize=8,
               color=(COLORS[2], 0.8),
               marker=:diamond,
)
...
save(joinpath(DIR, "plot.pdf"), fig, pt_per_unit=1)
