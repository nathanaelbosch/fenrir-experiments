...
# run(`scp uni:\~/PhD-Projects/fenrir/code/experiments/1/results/\*.txt $(RESULTS_DIR)`)
...
for (j, (problem, probname)) in enumerate(PROBLEMS)
    ...
    function trajectory_rmse(prob, p, tsteps)
        if length(p) > P
            _u0 = p[1:D]
            _p = p[D+1:end]
            sol = solve(remake(prob, u0=_u0, p=_p),
                        RadauIIA5(), abstol=1e-10, reltol=1e-10, saveat=tsteps, maxiters=1e7)
            return sqrt(mean(norm.(sol.u - true_sol.u) .^ 2))
        else
            sol = solve(prob, RadauIIA5(),
                        abstol=1e-10, reltol=1e-10, p=p, saveat=tsteps, maxiters=1e7)
            return sqrt(mean(norm.(sol.u - true_sol.u) .^ 2))
        end
    end
    ...
    get_prmses(params) = map(
        p -> length(p) > P ? norm(p[D+1:end] - prob.p) : norm(p - prob.p), params)
    ...
    trmses = Dict([(@info "Compute tRMSEs" k; k => get_trmses(v))
                   for (k, v) in parameters])
    # trmses = Dict([k => [l for l in v if l < 1e6] for (k, v) in trmses])
