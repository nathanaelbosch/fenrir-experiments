...
for (i, _prob) in enumerate((prob1, prob2, prob3, prob4))
    ...
    for j in 1:N_RUNS
        ...
        u0, κ²0 = get_initial_u0_diff(_prob, noisy_ode_data, tsteps)
        u0 = min.(max.(u0, u0_bounds[1]), u0_bounds[2])
        p0 = θ_init()
        σ²0 = 1.0
        ...
        l, times, states = exact_nll(θ_start, opt_params);

        f = OptimizationFunction(exact_nll, GalacticOptim.AutoForwardDiff())
        optprob = OptimizationProblem(
            f, θ_start, opt_params;
        ...
        opt_u0, opt_p, opt_σ², opt_κ² = optsol.u[1:D], optsol.u[D+1:end-2], exp(optsol.u[end-1]), exp(optsol.u[end])
